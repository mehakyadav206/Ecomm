...
const Products = () => {
  ...
  return (
    <Layout>
        ...
        <NewProduct addProduct={addProduct} categories={categories} update={updateProduct} />
      </div>
              ...
              {data.map((item) => (
                <tr key={item._id} className="border border-x-0 border-gray-300">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <span className='font-bold text-black'>{item.price}</span> / <span className='text-xs'>{item.mrp}</span></td>
